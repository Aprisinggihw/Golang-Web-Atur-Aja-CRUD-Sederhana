...
func GetAll() []entities.Product {
	...
	for rows.Next() {
		...
		if err != nil {
			fmt.Println("Error rows scan products in func Add")
			panic(err)
		}
...
func Add(product entities.Product) bool {
	...
	if err != nil {
		fmt.Println("Error inserting product into database: ")
		panic(err)
	}

	lastInserId, err := result.LastInsertId()
	if err != nil {
		fmt.Println("Error lastInsertId")
		panic(err)
	}
...
func Detail(id int) entities.Product {
	...
	if err != nil {
		fmt.Println("Error rows scan products in func Detail")
		panic(err)
	}
	return product
}
...
func Update(id int, product entities.Product) bool{
	...
	if err != nil {
		fmt.Println("Error update products")
		panic(err)
	} 
	result, err := query.RowsAffected()
	if err != nil {
		fmt.Println("Error query.RowsAffected")
		panic(err)
	}
	return result > 0
}
